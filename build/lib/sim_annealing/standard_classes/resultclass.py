...
class AnnealerResult(object):
    ...
    def __call__(self):
        ...
        print(
            "{:<25s}{:<23s}".format(
                "Total time elapsed:", time_string(self.time_elapsed)
            ),
            "\n",
        )
    ...
    def comparison_known_param(self, act_param, act_ll, closeness=None):
        ...
        if closeness is None:
            limit = self.annealing_param["eps"]
        else:
            limit = closeness

        self.distance_optimal_f = self.energy - act_ll
        self.distance_optimal_p = self.state - act_param
